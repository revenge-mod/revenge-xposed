...
class CacheModule : Module() {
    ...
    override fun onCreate(activity: Activity) = with(activity) {
        ...
        NativeBridgeModule.registerMethod("revenge.caches.modules.read") {
            if (::modulesCache.isInitialized) modulesCache.toMap() else null
        }

        NativeBridgeModule.registerMethod("revenge.caches.modules.write") {
        ...
        NativeBridgeModule.registerMethod("revenge.caches.assets.read") {
            if (::assetsCache.isInitialized) assetsCache.toMap() else null
        }


        NativeBridgeModule.registerMethod("revenge.caches.assets.write") {
            val (data) = it
            @Suppress("UNCHECKED_CAST")
            assetsCache =
...
data class AssetsCache(
    ...
    fun toMap(): Map<String, Any> {
        return mapOf(
            "data" to data.mapValues {
                (_, mappings) -> mappings.map { (type, id) -> type to id }.toMap()
            },
            "version" to VERSION
        )
    }
